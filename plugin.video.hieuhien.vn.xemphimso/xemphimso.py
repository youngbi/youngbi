...
logo = xbmc.translatePath('special://home/addons/plugin.video.hieuhien.vn.xemphimso/icon.png')
searchlink = 'http://xemphimso.com/tim-kiem/'

addon = xbmcaddon.Addon()
addonID = addon.getAddonInfo('id')
addonname = addon.getAddonInfo('name')
mysettings = xbmcaddon.Addon(id='plugin.video.hieuhien.vn.xemphimso')
...
def addDir(name, url, mode, iconimage,edit,inum,gname):
    ...
    contextmenuitems = []
    if edit:
        contextmenuitems.append(('[COLOR red]Delete[/COLOR] [COLOR ffffd700]Search[/COLOR]','XBMC.Container.Update(%s?url=%s&mode=8&name=%s&inum=%d)'%('plugin://plugin.video.hkn.xemphimso',urllib.quote_plus(url),urllib.quote_plus(name),inum)))
        contextmenuitems.append(('[COLOR ff00ff00]Edit[/COLOR] [COLOR ffffd700]Search[/COLOR]','XBMC.Container.Update(%s?url=%s&mode=9&name=%s&inum=%d)'%('plugin://plugin.video.hkn.xemphimso',urllib.quote_plus(url),urllib.quote_plus(name),inum)))
...
def addLink(name,url,mode,iconimage,gname):
    ...
    contextmenuitems.append(('[COLOR FF32CD32]Add Movie To Library[/COLOR]','XBMC.Container.Update(%s?url=%s&gname=%s&mode=10)'%('plugin://plugin.video.hkn.xemphimso',urllib.quote_plus(url),urllib.quote_plus(gname))))
    contextmenuitems.append(('[COLOR FF32CD32]Add Season To Library[/COLOR]','XBMC.Container.Update(%s?url=%s&gname=%s&mode=11)'%('plugin://plugin.video.hkn.xemphimso',urllib.quote_plus(url),urllib.quote_plus(gname))))
    liz.addContextMenuItems(contextmenuitems,replaceItems=False)
