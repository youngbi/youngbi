...
def mess(message, timeShown=10000,title=''):
	xbmc.executebuiltin((u'XBMC.Notification(%s,%s,%s,%s)'%('[COLOR lime][B]Kho Phim Xshare[/B][/COLOR] [COLOR green]%s[/COLOR]'%title,message,timeShown,icon['icon'])).encode("utf-8"))

def mess_yesno(title='[COLOR green]Kho Phim Xshare[/COLOR]', line1='', line2=''):
...
def make_myFshare(name,url,img,fanart,mode,query):#11
	...
	if not myFshare or (myFshare=='RDA4FHXVE2UU' and myaddon.getSetting('usernamef')!='thai@thanhthai.net'):
		...
		if folder:
			...
			traloi=mess_yesno('Kho Phim Xshare - Cảnh báo nguy hiểm!',line1,line2)
...
def vp_update(auto=True):#vp_2fshare(url):id,title,href,img,fanart,category
	mess(u'[COLOR green]Kho Phim Xshare database updating ...[/COLOR]',title='[COLOR gold]%s[/COLOR]'%homnay)
...
def fptplay(name,url,img,mode,page,query):
	...
	def login():
		...
		if 'laravel_value' in response.cookiestring:
			mess(u'[COLOR green]FPTPlay Movie Loading![/COLOR]');f=response.cookiestring
...
def megabox(name,url,img,fanart,mode,page,query):
	...
	def get_detail(urls,dict):
		mes('[COLOR green]Kho Phim Xshare database updating ...[/COLOR]')
		for url in urls:
	...
	def load_urls(urls):
		...
		if urls_new:endxbmc();json_rw(get_detail(urls_old,dict));mes('[COLOR lime]Kho Phim Xshare database updated[/COLOR]')
	...
	def update_href_new(hrefs,dict):
		...
		for href in href_new:
			...
			dict[id]=(series,name,img,fanart,views,esp,tm,p18)
		json_rw(dict);mes('[COLOR lime]Kho Phim Xshare database updated[/COLOR]')
...
def hdviet(name,url,img,mode,page,query):
	...
	def login_hdviet():
		...
		if response and response.get('error')==0:
			...
			mess(u'[COLOR green]HDViet Movie Loading![/COLOR]',title='%sHDViet.com[/COLOR]'%color['hdviet'])
			json_rw('hdviet.cookie',response)
		elif response and response.get('error')==27:
			mess(u'[COLOR red][B]Đăng ký TK Free[/COLOR][COLOR lime] hdviet.com [/COLOR][COLOR red] để dùng nhé![/B][/COLOR]',title='%sHDViet.com[/COLOR]'%color['hdviet']);response=dict()
	...
	def login():
		...
		try:
			if response.json['success']=='success':
				mes(u'[COLOR green]HayHayTV Movie Loading![/COLOR]');f=response.cookiestring
...
def phimmoi(name,url,img,mode,page,query):
	...
	def login_pm():
		...
		if response.status==302 and makerequest(joinpath(datapath,'phimmoi.cookie'),response.cookiestring,'w'):
			mes(u'[COLOR green]PhimMoi Movie Loading![/COLOR]');f=response.cookiestring
