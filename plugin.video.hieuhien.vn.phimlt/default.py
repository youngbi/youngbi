...
addonID = "plugin://plugin.video.hieuhien.vn.phimlt"
plugin  = Plugin()
listMax = 28
...
def Home() :
	items = [
	{ 'label' : '[COLOR white]Phim mới[/COLOR]', 'path' : '%s/latest/%s/%s' %(addonID, urllib.quote_plus('http://phimlt.com/list/new/p%s'), 1) },
	{ 'label' : '[COLOR white]Phim hot[/COLOR]', 'path' : '%s/hot/%s/%s' %(addonID, urllib.quote_plus('http://phimlt.com/list/top-rate/p%s'), 1) },
	{ 'label' : '[COLOR white]Phim xem nhiều[/COLOR]', 'path' : '%s/popular/%s/%s' %(addonID, urllib.quote_plus('http://phimlt.com/list/phim-xem-nhieu/p%s'), 1) },
	{ 'label' : '[COLOR aqua]Phim lẻ[/COLOR]', 'path' : '%s/movies/%s/%s' %(addonID, urllib.quote_plus('http://phimlt.com/phim-le/p%s'), 1) },
	{ 'label' : '[COLOR aqua]Phim bộ[/COLOR]', 'path' : '%s/series/%s/%s' %(addonID, urllib.quote_plus('http://phimlt.com/phim-bo/p%s'), 1) },
	{ 'label' : '[COLOR blue]Theo thể loại[/COLOR]', 'path' : '%s/genres' % addonID},
	{ 'label' : '[COLOR blue]Theo Quốc gia[/COLOR]', 'path' : '%s/nations' % addonID},
...
def LoadVideos(url):
	...
	if "nonHD=" in pageContent :
		source = re.compile('nonHD="(.+?)"').findall(pageContent)[0]
		if plugin.get_setting('HQ', bool) and "&hd=1" in pageContent :
			source += "&hd=1"
	if '<source src=' in pageContent:
		source = GetVideo(pageContent)
	if 'iframe id="iplayer"' in pageContent :
		links = [url]
		if '<div class="mirrors">' in pageContent:
			match = re.compile('<div class="mirrors">(.+?)</div>').findall(pageContent)[0]
			links.extend(re.compile('<a class="button" href="(.+?)"[^>]*>').findall(match))
		for i in range(len(links)):
			if i > 0:
				pageContent = GetUrl(links[i])
			frame = re.compile('<iframe id="iplayer" class="smallVid" frameborder="0" allowtransparency="true" scrolling="no" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" src="(.+?)">').findall(pageContent)[0]
...
def sortedItems(url, page, route_name):
	...
	match = re.compile('<div class="inner"><div class="pic"><a href="(.+?)" title="(.+?)"><img class="thumb" src="(.+?)"[^>]*/>').findall(pageContent)
	items = []
...
def Servers(murl):
	...
	match = re.compile('<img src="http://phimlt.com/img/icon_server.png" alt="Icon server phim"[^>]*/>(.+?)\:</span><ul class="episodelist-info-page">(.+?)</ul>').findall(pageContent)
	episodeName = re.compile('<title>(.+?)</title>').findall(pageContent)[0]
	serverList = []
	for serverName, episodeList in match :
		serverEps = []
		for episodeUrl, episodePart in re.compile('<a[^>]*href="(.+?)"[^>]*>(.+?)</a>').findall(episodeList):
			episode = {}
			episode["url"] = episodeUrl
			episode["name"] = "Part %s - %s" %(episodePart, episodeName.split(" | ")[0])
